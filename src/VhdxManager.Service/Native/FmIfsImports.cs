...
internal static class FmIfsImports
{
	...
	// 6-arg FormatEx (no cluster size). Supported since Windows 2000.
	...
	// 8-arg FormatEx2 (with BackwardCompatible + ClusterSize). Available on
	// Windows Vista and later. Use this when a non-default cluster size is required.
	[DllImport("fmifs.dll", CharSet = CharSet.Unicode, ExactSpelling = true)]
	public static extern void FormatEx2(
		string driveRoot,
		FmIfsMediaType mediaType,
		string fileSystemName,
		string label,
		[MarshalAs(UnmanagedType.U1)] bool quickFormat,
		[MarshalAs(UnmanagedType.U1)] bool backwardCompatible,
		uint clusterSize,
		FmIfsCallback callback);
}
