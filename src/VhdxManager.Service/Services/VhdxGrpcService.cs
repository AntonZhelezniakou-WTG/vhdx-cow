	...
	ILogger<VhdxGrpcService> logger) : VhdxService.VhdxServiceBase
{
	...
	public override async Task<CreateChildReply> CreateChild(CreateChildRequest request, ServerCallContext context)
	{
		...
		try
		{
			...
			await virtDiskManager.CreateDifferencingDiskAsync(
			...
			var physicalPath = await virtDiskManager.AttachAsync(
	...
	public override async Task<ResetChildReply> ResetChild(ResetChildRequest request, ServerCallContext context)
	{
		...
		try
		{
			...
			// 2. Detach the VHDX
			await virtDiskManager.DetachAsync(request.ChildVhdxPath, context.CancellationToken);
	...
	public override async Task<PublishReply> Publish(PublishRequest request, ServerCallContext context)
	{
		...
		try
		{
			...
			foreach (var mount in allMounts)
			{
				...
				await virtDiskManager.DetachAsync(mount.ChildVhdxPath, context.CancellationToken);
				File.Delete(mount.ChildVhdxPath);
			}

			// 3. Detach the overlay itself
			await virtDiskManager.DetachAsync(request.OverlayVhdxPath, context.CancellationToken);

			// 4. Merge overlay into parent
			await virtDiskManager.MergeAsync(request.OverlayVhdxPath, context.CancellationToken);
	...
	public override async Task<CreateVhdxReply> CreateVhdx(CreateVhdxRequest request, ServerCallContext context)
	{
		...
		try
		{
			...
			await virtDiskManager.CreateStandaloneVhdxAsync(
			...
			else
			{
				...
				await virtDiskManager.DetachAsync(request.VhdxPath, context.CancellationToken);
				attached = false;
			}
		...
		catch (Exception ex)
		{
			...
			// Best-effort rollback: undo each step that succeeded. Failures here are
			// logged but never thrown — we don't want to mask the original error.
			if (mounted)
			{
				try { await volumeManager.UnmountFolderAsync(request.MountPath, CancellationToken.None); }
				catch (Exception undo) { logger.LogWarning(undo, "Rollback unmount failed for {Mount}", request.MountPath); }
			}
			if (attached)
			{
				try { await virtDiskManager.DetachAsync(request.VhdxPath, CancellationToken.None); }
				catch (Exception undo) { logger.LogWarning(undo, "Rollback detach failed for {Path}", request.VhdxPath); }
			}
			if (fileCreated)
			{
				try { if (File.Exists(request.VhdxPath)) File.Delete(request.VhdxPath); }
				catch (Exception undo) { logger.LogWarning(undo, "Rollback delete failed for {Path}", request.VhdxPath); }
			}
	...
	public override async Task<AttachAndMountReply> AttachAndMount(AttachAndMountRequest request, ServerCallContext context)
	{
		...
		try
		{
			var physicalPath = await virtDiskManager.AttachAsync(request.VhdxPath, context.CancellationToken);
