namespace VhdxManager.Service.Security;
...
public sealed class PathValidator(IConfiguration configuration, ILogger<PathValidator> logger)
{
	readonly string[] allowedParentPaths
		= ExpandEnvVars(configuration.GetSection("VhdxManager:AllowedParentPaths").Get<string[]>());

	readonly string[] allowedMountBasePaths
		= ExpandEnvVars(configuration.GetSection("VhdxManager:AllowedMountBasePaths").Get<string[]>());

	readonly string[] allowedChildBasePaths
		= ExpandEnvVars(configuration.GetSection("VhdxManager:AllowedChildBasePaths").Get<string[]>());
