	...
	: IFolderTransferOrchestrator
{
		...
		CancellationToken ct)
	{
		...
		try
		{
			...
			await vhdxManager.CreateStandaloneVhdxAsync(vhdxPath, sizeBytes, dynamic, ct);
			stage = ConvertStage.VhdxCreated;
			ct.ThrowIfCancellationRequested();

			physicalPath = await vhdxManager.AttachAsync(vhdxPath, ct);
		...
		string? physicalPath)
	{
		...
		try
		{
			...
			{
				vhdxManager.DetachAsync(vhdxPath, CancellationToken.None).GetAwaiter().GetResult();
			}
		}
