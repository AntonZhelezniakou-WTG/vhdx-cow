...
namespace VhdxCow.Service.Diagnostics;
...
public static class FatalDiagnostics
{
	...
	static string BuildReport(Exception exception)
	{
		...
		sb.AppendLine("VhdxCow Service — fatal startup failure");
		sb.AppendLine("========================================");
		...
		sb.AppendLine();
		sb.AppendLine("Please send this file to the VhdxCow maintainers (open an issue with this report attached).");
		return sb.ToString();
	}
	...
	static void TryWriteEventLog(string report, string? reportPath)
	{
		...
		if (TryWriteToSource("VhdxCow", message))
		{
			return;
		}

		_ = TryWriteToSource("Application", $"[VhdxCow] {message}");
	}
