...
public sealed class Upgrade_Tests : InstalledFixtureBase
{
	...
	protected override async Task OnGuestReadyAsync()
	{
		...
		_bumpedMsi = await BumpedMsiBuilder.BuildAsync(Msi, repoRoot);
		...
		var guestMsiPath = $@"C:\Setup\{_bumpedMsi.FileName}";
		await Guest.CopyToGuestAsync(_bumpedMsi.HostPath, guestMsiPath);
		_upgradeResult = await MsiInstaller.InstallSilentAsync(Guest, guestMsiPath);
	...
	{
		_upgradeResult.Succeeded.Should().BeTrue(
			$"msiexec /i {_bumpedMsi.FileName} returned exit code {_upgradeResult.ExitCode} " +
			$"(expected 0 for a MajorUpgrade from {_bumpedMsi.BaseVersion} → {_bumpedMsi.BumpedVersion}). " +
			$"Log: {_upgradeResult.LogPath}\nTail:\n{_upgradeResult.LogTail}");
	}

	[Test, Order(2)]
	public async Task Service_Running_After_Upgrade()
	{
		if (!_upgradeResult.Succeeded) Assert.Inconclusive("Upgrade step failed.");
...
");
		fileVersion.Should().StartWith(_bumpedMsi.BumpedVersion,
			$"vhmgr.exe FileVersion should match the upgraded version " +
			$"({_bumpedMsi.BumpedVersion}); got '{fileVersion}'. If this fails the upgrade " +
...
");
		fileVersion.Should().StartWith(_bumpedMsi.BumpedVersion,
			...
			$"({_bumpedMsi.BumpedVersion}); got '{fileVersion}'");
	}
