...
{
	MsiResult uninstallResult = null!;

	protected override async Task OnGuestReadyAsync()
	{
		var guestMsiPath = InstalledCheckpoint.GuestMsiPath(Msi);
		uninstallResult = await MsiInstaller.UninstallSilentAsync(Guest, guestMsiPath);
	}

	[Test]
	public void Msiexec_SilentUninstall_ExitsZero()
	{
		uninstallResult.Succeeded.Should().BeTrue(
			$"msiexec /x {Msi.FileName} returned {uninstallResult.ExitCode}. " +
			$"Guest log: {uninstallResult.LogPath}\nTail:\n{uninstallResult.LogTail}");
	}
	...
	public async Task Service_Is_Gone()
	{
		if (!uninstallResult.Succeeded)
			Assert.Inconclusive("MSI uninstall failed.");
	...
	{
		if (!uninstallResult.Succeeded) Assert.Inconclusive("MSI uninstall failed.");
