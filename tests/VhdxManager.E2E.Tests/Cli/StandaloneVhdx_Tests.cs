...
public sealed class StandaloneVhdx_Tests : InstalledFixtureBase
{
	...
	protected override async Task OnGuestReadyAsync()
	{
		...
		// before `vhdx create --mount` will attach to it.
	...
	public async Task Create_Standalone_Vhdx_And_Mount()
	{
		...
		r.Succeeded.Should().BeTrue(
			$"`vhdx create` returned {r.ExitCode}.\nstdout: {r.StdoutText}\nstderr: {r.StderrText}");
	...
	public async Task List_Shows_The_New_Mount()
	{
		...
		r.StdoutText.Should().Contain("standalone.vhdx",
			$"the just-created VHDX should appear in `vhdx list`. stdout: {r.StdoutText}");
		r.StdoutText.Should().NotContain("No active mounts");
	}

	// NOTE: there's no Status_Reports_Attached test here. `vhdx status
	...
	{
		var r = await Vhdx.RunAsync(Guest, $"unmount --path \"{VhdxPath}\"");
		r.Succeeded.Should().BeTrue(
			$"`vhdx unmount` returned {r.ExitCode}. stderr: {r.StderrText}");

		// File must still exist (unmount is non-destructive)…
		await GuestFs.AssertFileExistsAsync(Guest, VhdxPath);
		// …and list should be empty again.
		var list = await Vhdx.RunAsync(Guest, "list");
	...
	public async Task Mount_Reattaches_Existing_Vhdx()
	{
		...
		r.Succeeded.Should().BeTrue(
			$"`vhdx mount` returned {r.ExitCode}. stderr: {r.StderrText}");
	...
	public async Task Delete_Removes_The_Vhdx_File()
	{
		...
		var r = await Vhdx.RunAsync(Guest, $"delete \"{VhdxPath}\"");
		r.Succeeded.Should().BeTrue(
			$"`vhdx delete` returned {r.ExitCode}. stderr: {r.StderrText}");
