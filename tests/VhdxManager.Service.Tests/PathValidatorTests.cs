...
public class PathValidatorTests
{
		...
		string[]? childPaths = null)
	{
		var config = new ConfigurationBuilder()
			...
			{
				["VhdxManager:AllowedParentPaths:0"] = parentPaths?.ElementAtOrDefault(0) ?? @"C:\VhdxDisks\Parents",
				["VhdxManager:AllowedMountBasePaths:0"] = mountPaths?.ElementAtOrDefault(0) ?? @"C:\Mounts",
				["VhdxManager:AllowedChildBasePaths:0"] = childPaths?.ElementAtOrDefault(0) ?? @"C:\VhdxDisks\Children",
			})
			.Build();
